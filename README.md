...
**Возможности:** testnet/mainnet, исполнение ордеров, настраиваемые пороги Z-Score, логирование сделок, скрипт анализа для оптимизации после 2–3 недель работы.
...
- **Машина состояний** для сигналов (фиксированные пороги для бэктеста):
...
- *Примечание: live_bot использует настраиваемые пороги из config*
...
- **Настраиваемые пороги** Z-Score (вход, тейк-профит, стоп-лосс) через `.env`
...
- **Запись сделок** в `live_trades.csv` при каждом выходе (для анализа)
...
- **Пороги Z-Score** (Z_ENTRY_LONG, Z_ENTRY_SHORT, Z_EXIT_TP_*, Z_STOP_LOSS)
- `ENABLE_TRADING` — включение реального исполнения ордеров
- `LIVE_TRADES_FILE` — путь к файлу логирования сделок
...
# BINANCE_API_KEY, BINANCE_SECRET, TG_BOT_TOKEN, TG_CHAT_ID
...
**Все чувствительные данные** (API-ключи, токены) хранятся в `.env`. Файл `.env` в `.gitignore` и не попадает в репозиторий. Используйте `.env.example` как шаблон.
...
1. **Сделки** сохраняются в `live_trades.csv` при каждом выходе из позиции
2. **Анализ:** `python analyze_live_trades.py` (или `python analyze_live_trades.py путь/к/live_trades.csv`)
3. **Отчёт:** Win Rate, PnL по направлениям и причинам выхода, Z-Score при входе/выходе, рекомендации по порогам
4. **Настройка:** добавить в `.env` новые значения (Z_ENTRY_LONG, Z_ENTRY_SHORT и т.д.) и перезапустить бота

**Формат `live_trades.csv`:** timestamp, pair, direction, entry/exit prices, entry_z, exit_z, exit_reason, pnl_usd, pnl_pct, beta, amount_a, amount_b
...
| `.env` | Секреты и настройки (не в git, см. `.env.example`) |
